...
class ContentStudioAgent:
    ...
    async def create(
        ...
        title = (
            f"Top {len(picks)} Crypto {label}: "
            f"Next {self.settings.prediction_horizon_hours} Hours"
        )
    ...
    def _caption(self, direction: Direction, picks: list[FinalPick]) -> str:
        ...
        lines = [
            (
                f"Top {len(picks)} Crypto {label} for the next "
                f"{self.settings.prediction_horizon_hours} hours"
            ),
            "",
