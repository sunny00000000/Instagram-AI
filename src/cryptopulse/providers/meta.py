...
class MetaPublisher:
    ...
    def _dry_run_records(self, package: ContentPackage) -> list[PublishRecord]:
        ...
        if self.settings.publish_instagram:
            ...
            if self.settings.publish_stories:
                story_count = sum(
                    asset.content_type == "story" for asset in package.assets
                )
                for index in range(1, story_count + 1):
                    records.append(
                        PublishRecord(
                            ...
                            success=True,
                            external_id=(
                                f"dry-run-ig-story-{index}-{package.run_id}-"
                                f"{package.direction.value}"
                            ),
                            published_at=datetime.now(UTC),
                        )
                    )
            if self.settings.publish_reels and any(
                asset.content_type == "reel" for asset in package.assets
            ):
                records.append(
                    PublishRecord(
                        platform="instagram",
                        ...
                        direction=package.direction,
                        success=True,
                        external_id=f"dry-run-ig-reel-{package.run_id}-{package.direction.value}",
                        published_at=datetime.now(UTC),
                    )
                )
        if self.settings.publish_facebook:
